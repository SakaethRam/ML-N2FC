...
class GameplayEncoder(nn.Module):
    ...
    def __init__(self, config: NCFNConfig):
        ...
        # Each scalar feature is treated as a token; we first embed each to D_mid2
        # via a learned position-wise linear, then apply self-attention.
        # Shape: [B, D_in] → [B, D_in, D_mid2] → attention → [B, D_in * D_mid2] would be huge
        # Instead: project to D_mid first, then treat D_mid tokens as sequence of 1 token
        # We project to D_mid2, run attention over a single-token, then project to D_out.
        ...
        # We reshape into NUM_FEATURE_TOKENS tokens of FEAT_TOKEN_DIM each
        # where NUM_FEATURE_TOKENS * FEAT_TOKEN_DIM = D_mid2
...
class ChatEncoder(nn.Module):
    ...
    def __init__(self, config: NCFNConfig):
        ...
        # indices 0=sentiment, 1=hype, 2=toxicity
        GATE_DIMS = 3
